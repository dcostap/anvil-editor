...
---Indicates the last time this instance updated its instance file.
---@field last_update integer
---The messages been broadcasted.
---@field messages plugins.ipc.message[]
---The replies been broadcasted.
---@field replies plugins.ipc.reply[]
---Root Project path opened in the instance.
---@field root_project string?
---All Project paths loaded in the instance.
...
local function current_project_paths()
  local paths = {}
  for _, project in ipairs(core.projects or {}) do
    if project and project.path then
      paths[#paths + 1] = project.path
    end
  end
  return paths
end

local function instance_has_project(instance, project_path)
  if type(project_path) ~= "string" then return false end
  for _, path in ipairs(instance.projects or {}) do
    if common.path_equals(path, project_path) then return true end
  end
  return common.path_equals(instance.root_project, project_path)
end
...
---Updates the instance status of an IPC object.
function IPC:update_status()
  local project_paths = current_project_paths()
...
---Starts and registers the IPC instance and monitoring.
function IPC:start()
...
---Stop and unregister the IPC instance and monitoring.
function IPC:stop()
  self.running = false
  if not self.shmem then
    os.remove(self.file)
          ...
          if file_info and file_info.type == "file" then
            ...
            if instance and instance.id ~= self.id then
              if instance.last_update + 5 > os.time() then
                ...
                -- Delete expired instance file maybe result of a crash
                os.remove(path)
              end
...
---Find a running instance that already has the given Project loaded.
...
---Main IPC instance for current instance.
---@type plugins.ipc
local ipc = IPC()

---Get the IPC instance for the running anvil instance.
---@return plugins.ipc
...
--------------------------------------------------------------------------------
-- Override system.show_fatal_error to be able and destroy instance file on crash.
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Override core.exit to destroy ipc instance file on exit.
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Avoid opening duplicate Project windows when another IPC instance already
-- advertises the same Project path under platform path-identity semantics.
--------------------------------------------------------------------------------
...
function core.open_project_in_new_window(project, ...)
  ...
  return core_open_project_in_new_window(project, ...)
end

--------------------------------------------------------------------------------
-- Register methods for opening files and directories.
--------------------------------------------------------------------------------
ipc:register_method("core.raise_window", function()
