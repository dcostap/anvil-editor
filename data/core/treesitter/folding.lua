...
local function clamp_line(buffer, line)
  return common.clamp(math.floor(tonumber(line) or 1), 1, #buffer.lines)
end

local function normalize_query_position(buffer, line1, col1, line2, col2)
  ...
  local text = buffer.lines[line1] or ""
...
local function range_to_fold_target(buffer, node)
  if not buffer or not buffer.lines or not node then return nil end
  local line1 = clamp_line(buffer, node.start_line)
  local line2 = clamp_line(buffer, node.end_line)
  ...
  local col2
  if line2 == clamp_line(buffer, node.end_line) and end_col > 1 then
    col2 = end_col
  else
    col2 = #(buffer.lines[line2] or "") + 1
  end

  return {
    line1 = line1,
    col1 = common.clamp(col1, 1, #(buffer.lines[line1] or "") + 1),
    line2 = line2,
    col2 = common.clamp(col2, 1, #(buffer.lines[line2] or "") + 1),
    kind = "syntax",
    metadata = {
      provider = "treesitter",
      node_type = node.type,
...
function folding.get_fold_target(buffer, line1, col1, line2, col2, opts)
  if not buffer or not buffer.lines then return empty("no-buffer") end
  line1, col1, line2, col2 = normalize_query_position(buffer, line1, col1, line2, col2)
  if not line1 then return empty("no-selection") end
  local ranges, reason = selection.get_node_ranges(buffer, line1, col1, line2, col2, opts)
  ...
  for _, node in ipairs(ranges) do
    if is_foldable_node(node) then
      local target = range_to_fold_target(buffer, node)
