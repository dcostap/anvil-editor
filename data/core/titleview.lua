...
function TitleView:get_tabs_node()
  if not core.root_view then return nil end
  local node = core.root_view:get_active_node()
  if node and not node.locked then return node end
  if core.last_active_view then
    node = core.root_view.root_node:get_node_for_view(core.last_active_view)
    if node and not node.locked then return node end
  end
...
function TitleView:has_window_focus()
...
function TitleView:get_titlebar_tab_width(node, available_width)
...
function TitleView:get_titlebar_tab_rect(node, idx)
...
function TitleView:get_titlebar_tab_close_rect(node, idx)
...
function TitleView:get_titlebar_tab_at(px, py)
...
function TitleView:draw_titlebar_tabs()
...
function TitleView:on_mouse_pressed(button, x, y, clicks)
  TitleView.super.on_mouse_pressed(self, button, x, y, clicks)
  if button == "left" then
  ...
  elseif self.hovered_tab_index then
    ...
    if node and idx == self.hovered_tab_index then
      if button == "middle" or (button == "left" and close) then
        node:close_view(core.root_view.root_node, node.views[idx])
...
function TitleView:on_mouse_wheel(y, x)
