...
**Lifecycle and render-cache slices completed July 10, 2026.** `Doc` now publishes batched filename/syntax/close events; the shared semantic model follows eligibility and closes automatically; and each split `DocView` owns independent automatic Live Preview attach/detach lifecycle. `DocView` also caches line-provider output, supports targeted line invalidation, and maintains unwrapped variable heights through a range-updatable Fenwick index, so ordinary caret movement recomputes only old/new lines. Rendered fragments now also participate in wrapping while preserving source-column breaks, row-local drawing, and hit testing. Pointer drag and IME composition freeze the render snapshot; disjoint multi-cursors reveal independently; and targeted height changes above the viewport preserve its screen anchor. Contracts and red-green evidence are recorded in `MARKDOWN_LIFECYCLE.md`, `MARKDOWN_RENDER_CACHE.md`, `MARKDOWN_WRAPPING.md`, and `MARKDOWN_INTERACTION_STABILITY.md`. Rendered widgets now use generic DocView hover/click routing, and Markdown images no longer patch global mouse methods; the contract is recorded in `MARKDOWN_WIDGET_INPUT.md`. Semantic-provider migration and broader POI/link adoption remain before the Phase 2 exit gate.
