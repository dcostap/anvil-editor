...
**Backend, incremental-model, and Obsidian-extension spikes completed July 10, 2026.** Tree-sitter Markdown 0.5.3 was selected and the evidence is recorded in `MARKDOWN_PARSER_BACKEND.md`. The worker-backed composite parser, revision-checked per-Document semantic model, exact-range compatibility fixtures, request coalescing/cancellation, pending raw fallback, persistent incremental block/inline reuse, edit-mapped stable semantic IDs, coalesced changed-line publication, indexed bounded line queries, and native Wikilink/embed/highlight/comment layer are implemented. Repeatable 100 KiB/1 MiB end-to-end measurements are recorded in `MARKDOWN_OBSIDIAN_EXTENSIONS.md`; 100 KiB incremental native total is 19–20 ms and therefore remains slightly above the initial 16 ms target before the Phase 1 performance gate is considered complete.
