...
test.describe("Buffer Registry", function()
  ...
  test.it("rejects a duplicate file Buffer without a partial record", function()
    local registry = BufferRegistry()
    local one = buffer("C:/work/same.lua")
    local duplicate = buffer("c:/WORK/same.lua")
    test.equal(registry:register(one), one)
    local result, err = registry:register(duplicate)
    test.is_nil(result)
    test.ok(err)
    test.equal(#registry:list(), 1)
    test.not_ok(pcall(registry.retain, registry, duplicate, {}))
  end)
end)
