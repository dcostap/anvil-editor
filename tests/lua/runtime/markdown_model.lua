...
local function make_buffer(text, filename)
...
test.describe("Markdown semantic model", function()
  test.it("completes fenced-code info metadata from a body-line lookup", function()
    local buffer = make_buffer("```javascript title=example\nconst value = 1\n```\n")
  ...
  test.it("shares one asynchronously published model per Buffer", function()
    local buffer = make_buffer("# Heading\n\nText with **bold** and *italic*.\n")
    local first = markdown_model.get(buffer)
    local second = markdown_model.get(buffer)
    test.equal(first, second)
    test.equal(first.status, "pending")
    test.ok(wait_status(first, "ready"), first.reason)
    test.equal(first.published_revision, buffer.text_revision)
  ...
  test.it("preserves semantic identities through incremental publications", function()
    ...
    buffer:insert(#buffer.lines, 1, "Tail paragraph.\n")
    ...
    test.ok(instance.changed_ranges[1].line1 <= 4)
    test.ok(instance.changed_ranges[1].line2 >= 5)
  ...
  test.it("dispatches edits without a debounce delay", function()
    local buffer = make_buffer("# Heading\n\nBody\n")
    local instance = markdown_model.get(buffer)
    test.ok(wait_status(instance, "ready"), instance.reason)
  ...
  test.it("publishes the semantic range affected by a Setext marker", function()
    local buffer = make_buffer("Title\n\nBody\n")
    local instance = markdown_model.get(buffer)
    test.ok(wait_status(instance, "ready"), instance.reason)
    ...
    local changed = test.not_nil(instance.changed_ranges[1])
    test.equal(changed.line1, 1)
    test.ok(changed.line2 >= 2)
    markdown_model.close(buffer, "test")
  end)

  test.it("keeps pending and incomplete syntax on the raw-fallback path", function()
    local buffer = make_buffer("Incomplete **bold\n")
    local instance = markdown_model.get(buffer)
  ...
  test.it("rejects stale parse publication after a newer text revision", function()
    ...
    test.equal(instance.published_revision, buffer.text_revision)
  ...
  test.it("closes native results rejected during publication", function()
    local buffer = make_buffer("# Heading\n")
    local instance = markdown_model.get(buffer, { ensure = false })
    local closed = 0
    local result = { close = function() closed = closed + 1; return true end }

    test.equal(instance:publish(result, buffer.text_revision + 1, "stale", instance.parse_generation), false)
    test.equal(closed, 1)
    markdown_model.close(buffer, "test")
  end)

  test.it("publishes exact Obsidian Wikilink and embed semantics", function()
    local buffer = make_buffer("[[Note#Heading|Alias]] and ![[image.png|640x480]]\n")
    local instance = markdown_model.get(buffer)
    test.ok(wait_status(instance, "ready"), instance.reason)
    local nodes = test.not_nil(instance:nodes_for_lines(1, 1))
  ...
  test.it("publishes highlights and multiline comments with code suppression", function()
    local buffer = make_buffer("==highlight **bold**== and `==code==`\n%%hidden **not bold**\nstill hidden%%\n")
    local instance = markdown_model.get(buffer)
    test.ok(wait_status(instance, "ready"), instance.reason)
  ...
  test.it("leaves escaped and incomplete Obsidian delimiters raw", function()
    local buffer = make_buffer("\\[[Escaped]] [[Incomplete\n\\==escaped==\n==incomplete\n[[]] [[|Alias]] %%%%\n")
    local instance = markdown_model.get(buffer)
    test.ok(wait_status(instance, "ready"), instance.reason)
  ...
  test.it("republishes after legacy raw edits and reload", function()
    local buffer = make_buffer("# Original\n")
    local instance = markdown_model.get(buffer)
    test.ok(wait_status(instance, "ready"), instance.reason)
    local generation = instance.generation

    buffer:raw_insert(1, 1, "prefix\n", buffer.undo_stack, system.get_time())
    test.ok(wait_status(instance, "ready"), instance.reason)
  ...
  test.it("cancels an in-flight parse when the model closes", function()
  ...
  test.it("dispatches a metadata edit after the text edit", function()
    local buffer = make_buffer("# Heading\n", "note.md")
    local instance = markdown_model.get(buffer)
    test.ok(wait_status(instance, "ready"), instance.reason)
    instance.diagnostics.last_total_ms = math.huge
    ...
    buffer:set_filename("moved.md", "moved.md")
    test.ok(wait_status(instance, "ready"), instance.reason)
    coroutine.yield(0.03)
    local pool = worker_pool.current_system()
    if pool then pool:drain({ max_ms = 5, max_messages = 64 }) end
    test.equal(instance.diagnostics.requests, requests + 2)
    markdown_model.close(buffer, "test")
  end)

  test.it("keeps active edit ranges unsafe for published rendering", function()
  ...
  test.it("does not allocate models for non-Markdown Buffers", function()
    local buffer = make_buffer("# Plain text", "note.txt")
    test.equal(markdown_model.get(buffer), nil)
    test.equal(markdown_model.peek(buffer), nil)
  end)
end)
