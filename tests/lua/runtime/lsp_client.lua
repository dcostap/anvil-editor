...
test.describe("core.lsp.client Phase 8.1 dispatch", function()
  ...
  test.test("advertises and answers workspace folders only when provided", function()
    local fake = fake_transport.new()
    local folders = lsp_json.array({ { uri = "file:///project", name = "project" } })
    local c = client.new(fake, { workspace_folders = folders })
    local params = c:initialize_params({ workspace_folders = folders })
    test.equal(params.capabilities.workspace.workspaceFolders, true)
    test.equal(params.workspaceFolders[1].name, "project")

    fake:push_message(jsonrpc.request(7, "workspace/workspaceFolders"))
    ...
    test.equal(responses[1].result[1].uri, "file:///project")
  end)
