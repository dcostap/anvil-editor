...
test.describe("Git Log local changes", function()
  ...
  test.it("compares each part of a partially staged file against its own base", function()
    ...
    test.equal(#rows, 3)
    test.equal(rows[1].subject, "Local Staged Changes")
    test.equal(rows[2].subject, "Local Unstaged Changes")
    local layout = graph.layout(rows)
    test.ok(layout[2].incoming)
    test.ok(layout[3].incoming)
    local staged = open_diff(model, rows[1])
    local unstaged = open_diff(model, rows[2])
  ...
  test.it("separates a staged rename from later edits at the new path", function()
    ...
    local rows = refresh(model)
    test.equal(#rows, 3)
    test.equal(rows[1].changed_files[1].status, "renamed")
    test.equal(rows[2].changed_files[1].status, "modified")
    test.equal(rows[2].changed_files[1].old_path, "new.txt")
    local staged = open_diff(model, rows[1])
    local unstaged = open_diff(model, rows[2])
  ...
  test.it("shows staged additions and untracked files before the first commit", function()
    ...
    test.equal(rows[1].subject, "Local Staged Changes")
    test.equal(rows[2].subject, "Local Unstaged Changes")
    test.equal(rows[2].changed_files[1].path, "untracked.txt")
    local staged = open_diff(model, rows[1])
