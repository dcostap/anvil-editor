...
test.describe("Selection surrounding", function()
  ...
  test.it("surrounds selected Markdown in Live Preview and Source Mode", function(context)
    ...
    test.ok(command.perform("markdown:surround_bold"))
    ...
    test.ok(command.perform("markdown:surround_italic"))
    test.equal(buffer_text(buffer), "**alpha** _beta_\n")
    test.equal(selection_text(buffer), "beta")
  end)

  test.it("inserts and removes empty Markdown formatting pairs at the caret", function(context)
    local view, buffer = new_view(context, "alpha", "note.md")
    panes.present(view, { placement = "new", focus = true })
    markdown_live.set_source_mode(view, false, "selection-surround-test")
    buffer:set_selection(1, 3)

    test.ok(command.perform("markdown:surround_bold"))
    test.equal(buffer_text(buffer), "al****pha\n")
    test.same(buffer.selections, { 1, 5, 1, 5 })

    test.ok(command.perform("markdown:surround_bold"))
    test.equal(buffer_text(buffer), "alpha\n")
    test.same(buffer.selections, { 1, 3, 1, 3 })

    markdown_live.set_source_mode(view, true, "selection-surround-test")
    test.ok(command.perform("markdown:surround_italic"))
    test.equal(buffer_text(buffer), "al__pha\n")
    test.same(buffer.selections, { 1, 4, 1, 4 })
  ...
  test.it("offers Markdown surround commands only for Markdown text", function(context)
    ...
    test.equal(command.perform("markdown:surround_italic"), false)
