...
test.describe("Row Selection Mode", function()
  ...
  test.it("keeps marked rows selected during ordinary navigation", function()
    ...
    test.ok(command.perform("core:toggle_selected_row_marks"))
    ...
    test.ok(command.perform("core:toggle_selected_row_marks"))
    command.perform("core:move_to_next_line")
    test.same(view:get_selected_rows(), { 3 })
  end)
  ...
  test.it("toggles every row in a Shift selection", function()
    local view = make_view()
    command.perform("core:select_to_next_line")
    command.perform("core:select_to_next_line")
    test.same(view:get_selected_rows(), { 1, 2, 3 })
    ...
    command.perform("core:toggle_selected_row_marks")
    test.same(view:get_marked_rows(), { 1, 2, 3 })
    test.same(view:get_selected_rows(), { 1, 2, 3 })

    command.perform("core:toggle_selected_row_marks")
    test.same(view:get_marked_rows(), {})
    test.same(view:get_selected_rows(), { 3 })
  end)

  test.it("clears all marks while keeping the focused row selected", function()
    local view = make_view()
    command.perform("core:toggle_selected_row_marks")
    command.perform("core:move_to_next_line")
    command.perform("core:toggle_selected_row_marks")
