...
test.describe("graphics apis", function()
  ...
  test.test("renders to a temporary window", function()
    ...
    local translucent = canvas.new(4, 4, {255, 0, 0, 255}, true)
    renderer.draw_canvas_scaled(translucent, 48, 48, 4, 4, 0.5)
    ...
    local translucent_center = renwindow.get_color(window, 50, 50)
    test.ok(translucent_center[1] > 110 and translucent_center[1] < 145)
    test.ok(translucent_center[2] < 5)
    test.ok(translucent_center[3] < 5)
    test.equal(translucent_center[4], 255)
  end)
end)
