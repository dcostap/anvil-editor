...
test.describe("File Tree Project Path Roles", function()
  ...
  test.it("renames Project Path labels by editing File Tree section roots", function(context)
    local filetree = setup_project_paths(context)
    local external_root = find_line(filetree, "jdk-src/")
    test.not_nil(external_root)
    ...
    test.equal(plan.project_path_label_updates[1].label, "jdk-renamed")
    test.equal(filetree:operation_count(plan), 1)

    filetree:apply_plan(plan)
    local resolved = project_paths.resolve(context.external)
    test.not_nil(resolved)
    test.equal(resolved.entry.label, "jdk-renamed")
    test.ok(system.get_file_info(context.external), "renaming the Project Path label must not rename the real folder")
  end)

  test.it("removes Project Paths by deleting File Tree section roots", function(context)
    local filetree = setup_project_paths(context)
    local external_root = find_line(filetree, "jdk-src/")
    test.not_nil(external_root)
    ...
    local plan = filetree:plan_changes(false)
    test.not_nil(plan)
    test.equal(#plan.project_path_removals, 1)
    test.equal(plan.project_path_removals[1].label, "jdk-src")
    test.equal(filetree:operation_count(plan), 1)

    filetree:apply_plan(plan)
    test.is_nil(project_paths.resolve(context.external))
    test.ok(system.get_file_info(context.external), "removing the Project Path must not delete the real folder")
  end)
end)
