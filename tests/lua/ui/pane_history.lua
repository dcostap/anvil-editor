...
test.describe("Pane View history", function()
  ...
  test.it("keeps the Current View when replacement construction fails", function()
    ...
    test.same(panes.views(pane), { one })
  end)
  ...
  test.it("records each revisit without duplicating its live View", function()
    local pane = panes.create { factory = factory("one") }
    local one = pane.current_view
    local two = make("two")
    panes.present(two, { pane = pane })
  ...
  test.it("clears a forward branch while retaining its protected live View", function()
    ...
    local three = make("three")
    panes.present(protected, { pane = pane })
    panes.present(three, { pane = pane })
    test.equal(panes.back(pane), protected)
    test.equal(panes.back(pane), one)
