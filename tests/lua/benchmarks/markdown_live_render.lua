...
test.describe("Markdown live render benchmark", function()
  test.it("reports provisional source-topology latency", function()
    local doc = Doc("topology-benchmark.md", "topology-benchmark.md", true)
    doc:insert(1, 1, representative_source(1024 * 1024))
    doc:clear_undo_redo()
    local p95, p99 = measure(20, function()
      pending_projection.source_topology(doc.lines)
    end)
    print(string.format(
      "Markdown pending topology benchmark: bytes=%d lines=%d p95_ms=%.3f p99_ms=%.3f",
      #table.concat(doc.lines), #doc.lines, p95, p99
    ))
    test.ok(p95 >= 0)
    test.ok(p99 >= p95)
  end)
  ...
  test.it("reports cached viewport and caret-transition latency", function()
    ...
    print(string.format(
      ...
      #table.concat(doc.lines), #doc.lines,
  ...
  test.it("reports edit-to-pending-presentation latency", function()
    local old_enabled = config.markdown_live_editor
    config.markdown_live_editor = true
    local doc = Doc("pending-benchmark.md", "pending-benchmark.md", true)
    doc:insert(1, 1, representative_source(100 * 1024))
    doc:clear_undo_redo()
    local view = DocView(doc)
    view.size.x, view.size.y = 1200, 800
    ...
    for sample = 1, 100 do
      ...
      doc:set_selection(line, #doc.lines[line])
      local started = system.get_time()
      ...
      test.equal(render.source_text, (doc.lines[line] or ""):gsub("\n$", ""))
      test.ok(wait_ready(instance), instance.reason)
    end
    ...
    print(string.format(
      ...
      #table.concat(doc.lines), #values, p95, p99,
  ...
  test.it("reports wrapped edit-to-pending-presentation latency", function()
    local old_enabled = config.markdown_live_editor
    config.markdown_live_editor = true
    local doc = Doc("pending-wrapped-benchmark.md", "pending-wrapped-benchmark.md", true)
    doc:insert(1, 1, representative_source(100 * 1024))
    doc:clear_undo_redo()
    local view = DocView(doc)
  ...
  test.it("reports large structural edit-to-pending-presentation latency", function()
    local old_enabled = config.markdown_live_editor
    config.markdown_live_editor = true
    local doc = Doc("pending-structural-benchmark.md", "pending-structural-benchmark.md", true)
    doc:insert(1, 1, representative_source(1024 * 1024))
    doc:clear_undo_redo()
    local view = DocView(doc)
    view.size.x, view.size.y = 1200, 800
