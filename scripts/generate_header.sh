...
makestub() {
  grep '^LUA' | grep -v "$IGNORE_SYM" | sed -e "s/$sym_regex/static\t\2\t__anvil_fallback_\3\t(\4) { fputs(\"warning: \3 is a stub\", stderr); }/"
}
...
generate_header() {
  local LUA_PATH="$1"
  echo "#ifndef ANVIL_PLUGIN_API"
  echo "#define ANVIL_PLUGIN_API"
  echo "/**"
  echo "The anvil plugin API is quite simple. Any shared library can be a plugin file, so long"
  echo "as it has an entrypoint that looks like the following, where xxxxx is the plugin name:"
  echo '#include "anvil_plugin_api.h"'
  echo "int luaopen_anvil_xxxxx(lua_State* L, void* XL) {"
  echo "  anvil_plugin_init(XL);"
  echo "  ..."
  echo "  return 1;"
  echo "}"
  ...
  echo 'static  void  __anvil_fallback_luaL_openlibs    (lua_State *L) { fputs("warning: luaL_openlibs is a stub", stderr); }'
  echo

  echo "#define IMPORT_SYMBOL(name, ret, ...) name = (name = (ret (*) (__VA_ARGS__)) symbol(#name), name == NULL ? &__anvil_fallback_##name : name)"
  echo "static void anvil_plugin_init(void *XL) {"
