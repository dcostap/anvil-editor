...
The expected first-order saving is the removal of approximately 3.0-3.4 ms of repeated Lua reconstruction from a one-pane frame. The specific acceptance targets are:

- reduce clean one-pane `draw_emit_ms` from 6.14 ms to **3.5 ms or less on average** under comparable recording conditions;
- reduce the combined steady-state scope time for wrapped text, Draw Whitespace, and Indent Guides by at least **70%**;
...
2. whitespace markers;
3. source text;
4. search outlines, Diagnostic Underlines, and Line Hints;
5. Indent Guides from the current plugin wrapper;
6. Bracket Match and other later overlays according to their existing wrappers;
...
Packet commands store native `RenFont` pointers, so packet lifetime must retain every Lua Font or font group used by the packet.

Implement one of these safe ownership models:

1. registry references for each distinct Font/font-group Lua value, released by packet `__gc`; or
2. a userdata environment/uservalue table retaining the Font values for the packet lifetime.

Do not rely on the existing per-frame renderer font-reference table. Packets survive across frames.

A packet must be invalidated before replay when a retained font's size or relevant metrics change. Retaining a Font prevents a dangling pointer; it does not make stale geometry correct.
...
1. obtain the persistent rendered token sequence from `Highlighter:get_render_line(line)`;
...
The plugin should expose a first-party packet contributor with behavior equivalent to the current renderer:
...
`highlight_active = true` depends on the active caret. Keep the highlighted segment dynamic:

- cache normal guide commands;
- omit or split the active guide depth from the normal packet commands;
- draw the active-color guide as a small dynamic overlay;
- do not invalidate every visible line packet whenever the caret moves.

When `highlight_active = false`, which is the current default, all ordinary guide commands can replay from the packet.
...
- Tree-sitter/LSP render-token output changes;
- a render-token provider generation changes;
- syntax font selection changes.

Prefer a stable per-line render-token revision/identity over hashing every token string every frame. If the existing highlighter cannot provide one reliably, add a per-line render revision and bump it from `Highlighter:update_notify()` and language-intelligence invalidation.
...
- a syntax-font change alters wrapping.
...
- immediately release packet userdata on eviction when no other references exist;
...
2. calculate translated command bounds;
3. skip commands definitely outside the active clip where doing so is cheap;
4. append equivalent ordinary commands to the active frame buffer;
5. copy packet-owned text bytes into the frame command buffer;
6. preserve tab offsets and Font fallback arrays;
7. update ordinary rencache frame statistics;
8. update packet-specific replay statistics;
9. fail safely if command-buffer growth fails.
...
On allocation failure, return a normal Lua error or a recoverable nil/error result. The Document View should log quietly and render that line through the existing path for the frame.
...
- Implement packet builder, sealing, `__gc`, memory accounting, and font retention.
- Add text-known-bounds, rect, and rect-grid packet commands.
- Add command layers and Wrapped Visual Row indices.
- Add translated replay into ordinary rencache commands.
- Add diagnostics.
...
- Compile wrapped syntax text into the packet content layer.
...
Run the targeted wrapped-text tests and verify behavior for ASCII, tabs, UTF-8, ligatures, syntax Fonts, and wrap-boundary token splitting.
...
Take a short capture and verify the `draw_whitespace` scope has disappeared or become a small packet replay scope for eligible lines.
...
- Keep active-guide highlighting dynamic.
...
5. Tree-sitter/LSP render-token adoption;
6. wrap-width change;
7. Font/Zoom change;
8. theme reload;
9. runtime style mutation;
10. whitespace option change;
11. Indent Guide option change;
12. plugin/module reload;
13. view close;
14. LRU count eviction;
15. LRU byte eviction;
16. oversized-line bypass.
...
6. Record with the same target refresh and window dimensions.
7. Compare:
   - active cadence;
   - `total_ms`;
   - `draw_emit_ms`;
   - Document View draw time;
   - packet hit/miss/build/replay metrics;
   - Lua heap growth and collection-correlated stalls;
   - renderer command counts/bytes;
   - renderer-end time;
   - Present time.
8. Repeat with two visible Document Views to verify near-linear scaling and no cache cross-contamination.
9. Repeat once with `ANVIL_RENDERER=software` for backend correctness, not headline performance.
...
5. compare command type, text, Font identity, color, relative bounds, tab offset, row, layer, and order.
...
The caret moves within the same line. Text, whitespace, and normal guide packets remain valid. Current-line background, active guide, Bracket Match, and caret update dynamically.
...
All metric-dependent packets rebuild. No packet retains stale widths or stale Font pointers.
...
Each view uses its own width/layout packets. Editing invalidates both views correctly without sharing incompatible geometry.
...
**Risk:** a packet outlives its Font userdata.

**Mitigation:** packet-owned Lua references plus `__gc`; never store unretained native Font pointers.

### Excessive packet memory

**Risk:** scrolling through a large file retains all visited lines.

**Mitigation:** count/byte LRU, visible-line pinning, oversized-packet bypass, and packet byte diagnostics.
...
- packet Font and memory lifetime are safe;
- packet caches are bounded;
...
- a clean one-pane recording demonstrates the planned draw-time reduction;
