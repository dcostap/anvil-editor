...
- behavior while a fresh parse is pending: retain exact unchanged presentation, map safe text edits, and show source for uncertain changed lines
- shutdown and test isolation

`core.add_thread()` is cooperative and cannot preempt a long native parser call. Parsing stays on the worker. Fast models dispatch edits immediately. Slow models use debounce and cancellation.
...
**Completed July 10, 2026; edit projection simplified September 16, 2026.** Tree-sitter Markdown 0.5.3 is the sole presentation authority. The worker model now publishes native semantic change ranges. Exact unchanged renders survive edits, while uncertain changed lines show source until publication. Fast models dispatch immediately. Large models stay debounced, asynchronous, and cancellable.
...
- Implement the Buffer semantic model with revision checks, non-semantic edit projection, and bounded adoption work.
...
**Completed July 10, 2026.** `Doc` now publishes batched filename/syntax/close events; the shared semantic model follows eligibility and closes automatically; and each split `DocView` owns independent automatic Live Preview attach/detach lifecycle. `DocView` caches line-provider output, supports provider-widened transaction invalidation, and maintains unwrapped variable heights through a range-updatable Fenwick index. Rendered fragments participate in source-preserving wrapping, row-local drawing, and hit testing. Pointer drag and IME composition freeze the render snapshot; disjoint multi-cursors reveal independently; and targeted height changes above the viewport preserve its screen anchor. Rendered widgets use generic DocView hover/click routing, so Markdown images no longer patch global mouse methods. Finally, headings and inline emphasis adopt stable identities from async semantic publications while unchanged cached lines survive incremental edits and context-dependent Markdown suffixes remain correct. Contracts and red-green evidence are recorded in `MARKDOWN_LIFECYCLE.md`, `MARKDOWN_RENDER_CACHE.md`, `MARKDOWN_WRAPPING.md`, `MARKDOWN_INTERACTION_STABILITY.md`, `MARKDOWN_WIDGET_INPUT.md`, and `MARKDOWN_SEMANTIC_RENDERING.md`. The representative 100 KiB benchmark measured a 0.378 ms cached 60-line viewport query p95 and 0.148 ms caret-transition p95, below the Phase 2 targets. Broader semantic syntax and POI/link adoption continue in Phases 3 and 4.
...
**Context-aware asset-service, remote-policy, and attachment paste/drop slices completed July 10, 2026.** Local images now share bounded decoded assets by resolved path while missing references retain source/Project context; remote request identity includes policy/cache context; subscribers receive deterministic async invalidation; failures retry on index generations; external numeric-label and alt-plus-size forms are normalized with explicit alt semantics; and width×height dimensions are aspect-preserving bounding boxes. Remote images remain disabled by default, with explicit view-local one-shot loading and optional shared Project trust/revocation commands; loading, blocked, and unavailable references retain alt-aware visible states. Generic DocView file-drop and clipboard-paste routing now copies external or binary clipboard attachments to Obsidian/configured Project locations with collision-safe names, inserts Wikilink or Markdown source only after success, and directly links existing Project files. Contracts and regression evidence are recorded in `MARKDOWN_IMAGE_ASSETS.md` and `MARKDOWN_ATTACHMENT_WORKFLOW.md`.
...
**Phase 6 baseline completed July 10, 2026.** Ordered/unordered/nested list, task, and blockquote markers now compose with semantic inline rendering; source-preserving hard-break presentation and indented-code backgrounds cover the remaining common block paths; checked/unchecked tasks toggle exact source ranges through generic fragment input; Obsidian callouts receive semantic header/card presentation with safe unknown-type fallback and nested inline rendering; fenced code receives language/closing chrome and generic block backgrounds while content stays raw; thematic breaks render as semantic rules; parser-confirmed frontmatter receives source-preserving structured styling and conservative alias/tag metadata indexing; boundary-valid Obsidian tags receive native semantic ranges and source-preserving styling; CommonMark reference links resolve through bounded same-snapshot definitions; reference footnotes receive the selected source-preserving Live Preview treatment; and active block lines retain safe raw reveal. The contract and regression evidence are recorded in `MARKDOWN_BLOCK_PRESENTATION.md`.
