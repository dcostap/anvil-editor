...
Implemented July 10, 2026 as the fourth Phase 3 slice in `plans/done/MARKDOWN_LIVE_EDITOR_PLAN.md`.
...
A non-empty selection reveals only semantic constructs whose source ranges actually intersect the selection. Selecting ordinary text elsewhere on the same line therefore leaves bold, italic, links, and other inline presentation intact; an unordered-list marker changes to source only when the selected range touches the marker token itself. Heading source is revealed when a heading marker range is selected. A plain heading with no other localized constructs also uses the whole-line fallback for a non-empty selection on its line, so moving a selection through its title does not unexpectedly hide the heading markers. Headings containing other localized constructs retain their localized selection behavior. Setting `markdown_live_reveal_mode = "line"` explicitly retains unconditional whole-line reveal behavior for collapsed carets.
