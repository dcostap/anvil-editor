...
static int dirmonitor_check_thread(void* data) {
  ...
  for (;;) {
    ...
    while (monitor->length > 0 && monitor->notification_pending) {
      SDL_WaitCondition(monitor->condition, monitor->mutex);
    }
    bool stopped = monitor->length < 0;
    if (!stopped && monitor->length > 0) {
      /* The previous SDL_PushEvent failed. Preserve the unread backend
       * buffer and retry its wake notification instead of waiting forever or
       * overwriting it with a subsequent backend read. */
      monitor->notification_pending = true;
      notify = true;
    }
    ...
    int result = 0;
    if (!notify) {
      result = monitor->backend->get_changes(
        monitor->internal, monitor->buffer, sizeof(monitor->buffer)
      );

      SDL_LockMutex(monitor->mutex);
      if (monitor->length >= 0 && result != 0) {
        monitor->length = result;
        if (!monitor->notification_pending) {
          monitor->notification_pending = true;
          notify = true;
        }
      }
      stopped = monitor->length < 0;
      SDL_UnlockMutex(monitor->mutex);
      if (stopped) break;
    }
    ...
    if (notify) {
      CustomEvent event;
      SDL_zero(event);
      if (push_custom_event("dirmonitor", &event)) {
        SDL_LockMutex(monitor->mutex);
        monitor->notifications_pushed++;
        SDL_UnlockMutex(monitor->mutex);
      } else {
        SDL_LockMutex(monitor->mutex);
        monitor->notification_pending = false;
        SDL_UnlockMutex(monitor->mutex);
        SDL_Delay(10);
      }
    } else if (result == 0) {
      /* Some backends poll instead of blocking. Do not spin when they report
       * no changes, and never wake the app for an empty poll. */
      SDL_Delay(10);
    }
  }
  return 0;
...
static int f_dirmonitor_new(lua_State* L) {
  ...
  monitor->condition = SDL_CreateCondition();
  if (!monitor->mutex || !monitor->condition) {
    if (monitor->condition) SDL_DestroyCondition(monitor->condition);
    if (monitor->mutex) SDL_DestroyMutex(monitor->mutex);
    return luaL_error(L, "Unable to initialize dirmonitor synchronization: %s", SDL_GetError());
  }
...
static int f_dirmonitor_gc(lua_State* L) {
  ...
  SDL_LockMutex(monitor->mutex);
  monitor->length = -1;
  SDL_BroadcastCondition(monitor->condition);
  SDL_UnlockMutex(monitor->mutex);
  ...
  SDL_DestroyCondition(monitor->condition);
...
static int f_dirmonitor_check(lua_State* L) {
  ...
  if (monitor->length < 0)
    ...
    if (monitor->backend->translate_changes(monitor->internal, monitor->buffer, monitor->length, f_check_dir_callback, L) == 0) {
      monitor->length = 0;
      monitor->notification_pending = false;
      SDL_SignalCondition(monitor->condition);
    }
    lua_pushboolean(L, 1);
  } else
    lua_pushboolean(L, 0);
...
static int f_dirmonitor_diagnostics(lua_State* L) {
  struct dirmonitor* monitor = luaL_checkudata(L, 1, API_TYPE_DIRMONITOR);
  SDL_LockMutex(monitor->mutex);
  Uint64 notifications_pushed = monitor->notifications_pushed;
  bool notification_pending = monitor->notification_pending;
  int length = monitor->length;
  SDL_UnlockMutex(monitor->mutex);

  lua_createtable(L, 0, 3);
  lua_pushnumber(L, (lua_Number)notifications_pushed);
  lua_setfield(L, -2, "notifications_pushed");
  lua_pushboolean(L, notification_pending);
  lua_setfield(L, -2, "notification_pending");
  lua_pushboolean(L, length > 0);
  lua_setfield(L, -2, "has_changes");
  return 1;
}
...
static const luaL_Reg dirmonitor_lib[] = {
  ...
  { "diagnostics", f_dirmonitor_diagnostics },
