...
7. Publication checks parse generation, `Doc.text_revision`, filename, absolute filename, syntax identity, and continued Markdown eligibility. Published snapshots include the coalesced changed source-line range. When an in-flight parse is superseded, its active unsafe range is merged with the queued range and its structural-invalidity flag is preserved for the replacement request.
